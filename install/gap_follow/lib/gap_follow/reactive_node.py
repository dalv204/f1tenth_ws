...
class ReactiveFollowGap(Node):
    ...
    def __init__(self):
        ...
        self.drive_speed = 0.7 # set t8o drive slower while running tests
        self.drive_speed_backup = 0.7
        self.safe = False
    ...
    def odom_callback(self,msg):
        """ checks speed and adjusts for jerk """
        read_velocity = msg.twist.twist.linear.x
        if read_velocity == 0.0:
            self.drive_speed=0.5
        else:
            self.drive_speed = self.drive_speed_backup

    def reverse_car(self):
        """ didnt find anything so try reversing car """
